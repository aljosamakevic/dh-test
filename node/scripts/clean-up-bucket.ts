...
async function run() {
  ...
  // 3. Pick the first bucket or pick a specific by ID if needed
  const bucket: Bucket = buckets[0];
  console.log(`Selected bucket: "${bucket.name}" (ID: ${bucket.bucketId}, Files: ${bucket.fileCount})`);
  // --8<-- [end:get-buckets]

  // --8<-- [start:delete-all-files]
  // 4. Delete all files in the bucket
  ...
  // 5. Wait for the bucket to be confirmed empty in the backend
