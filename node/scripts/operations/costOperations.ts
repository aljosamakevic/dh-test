...
const calculateTotalOutstandingDebt = async (address: string, paymentStreams: PaymentStreamsResponse) => {
  ...
  const currentPriceIndex = await polkadotApi.query.paymentStreams.accumulatedPriceIndex();
  // Get latest block
  ...
  for (const { provider, providerType } of paymentStreams.streams) {
    ...
    if (providerType === 'bsp') {
      ...
      if (stream.isSome) {
        ...
        const rawDebt =
          ((currentPriceIndex.toBigInt() - dynamicRatePaymentStream.priceIndexWhenLastCharged.toBigInt()) *
