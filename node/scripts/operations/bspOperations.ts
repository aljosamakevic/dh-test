...
import { chain, account, publicClient, walletClient, polkadotApi } from '../services/clientService.js';
import { formatEther } from 'viem/utils';
import { polkadotApiBsp } from '../services/bspService.js';
// --8<-- [end:imports]

// --8<-- [start:fund-bsp-address]
export async function fundBspAddress(bspAddress: `0x${string}`, amount: bigint) {
...
export async function checkBspBalance(bspAddress: `0x${string}`) {
  // Query balance
  const balance = formatEther(await publicClient.getBalance({ address: bspAddress }));
...
export async function requestBspSignUp(bspAddress: string, multiaddresses: Binary[], capacity: bigint) {
